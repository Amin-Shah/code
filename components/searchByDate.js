...
import { AsyncStorage, Text, ScrollView, StyleSheet, View, Image } from 'react-native'
import { Container, Content, Card, Header, Item, Input, Button, CardItem, Icon, Separator, Right, ListItem, List, Body } from 'native-base';
...
class SearchByDate extends Component {
    ...
    searchDetails = (e) => {
        e.preventDefault();
        this.props.searchbydate(this.state.date)
    }
    searchHelper = () => {
        arr = [];
        const date = this.props.Search.searchByDate ? this.props.Search.searchByDate : [];
        const helper = date[0];
